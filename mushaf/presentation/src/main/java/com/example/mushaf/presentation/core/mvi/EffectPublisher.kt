 ...
 *
 * Mirrors `com.iti.presentation.core.mvi.EffectPublisher`. `:mushaf:presentation` and
 * `:presentation` are sibling modules with no dependency between them, so the contract is
 * duplicated by design — keep the two copies identical.
 */
